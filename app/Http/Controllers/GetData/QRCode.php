...
class QRCode extends Controller
{
    ...
    public function QRItems(Request $req) {
        try {
            ...
            $QR = DB::select('SELECT Pk_inventoryId FROM inventories LEFT JOIN locat_man ON inventories.Fk_locatmanId = locat_man.Pk_locatmanId LEFT JOIN location ON locat_man.Fk_locationId = location.Pk_locationId WHERE location_name = ?', ["$location"]);
