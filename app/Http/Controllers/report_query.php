...
class report_query extends Controller
{
    public function report_query(Request $request)
    {
        try {
            $data = Inventory::query()
            ->selectRaw('CONCAT_WS(" ", article_name, type_name, model, variety, details2) AS "desc", location_name, person_name, cost AS "costs", Quantity AS "qty", cost * Quantity AS "total", property_no, serial, acquisition_date, "3 years" AS warranty') // Set the warranty directly to "3 years"
